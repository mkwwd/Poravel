...
<template>
  <tr class="list_item">
        ...
        <td class="subject">
            <router-link :to="{ name: 'qna-view', params: { article_type: article.article_type, article_no: article.article_pk } }" class="article-title">
                {{ article.subject }}
            </router-link>
        </td>
        <td>{{ star(article.user_id) }}</td>
        <td>{{ article.hit }}</td>
        <td>{{ cut(article.register_time) }}</td>
    </tr>
</template>
